...
type MultiWeekResult = {
  weeks: MultiWeekEntry[];
  existingTechAssignments?: Record<string, string | null>;
};
...
export function WeeklyOptimizerPanel({
  ...
  const liveCredits =
    applyConfirmPending && freshCreditData != null ? freshCreditData.credits : credits;
  ...
  const analyzeMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const initialAccepted = new Set(data.weeks.filter((w) => !w.empty).map((w) => w.weekStart));
    ...
    >
      ...
      >
          ...
          >
            <div className="w-full max-w-sm space-y-4">
                    ...
                    <div className="shrink-0 w-5 h-5 flex items-center justify-center">
                      {ws.status === "pending" && (
                        <span
                          ...
                          data-testid={`icon-week-pending-${ws.weekStart}`}
                        />
                      )}
                      {ws.status === "applying" && (
                        <Loader2
                          className="h-4 w-4 animate-spin text-primary"
                          data-testid={`icon-week-applying-${ws.weekStart}`}
                        />
                      )}
                      {ws.status === "success" && (
                        <CheckCircle2
                          className="h-4 w-4 text-green-600 dark:text-green-400"
                          data-testid={`icon-week-success-${ws.weekStart}`}
                        />
                      )}
                      {ws.status === "error" && (
                        <XCircle
                          className="h-4 w-4 text-destructive"
                          data-testid={`icon-week-error-${ws.weekStart}`}
                        />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {ws.status === "error" && ws.error && (
                        <p
                          ...
                          data-testid={`text-week-error-${ws.weekStart}`}
                        >
                          {ws.error}
                        </p>
                      )}
              ...
              <div
                ...
                data-testid="progress-bar-apply"
              >
                <div
              ...
              <p className="text-center text-xs text-muted-foreground">
                {
                  weekApplyStatuses.filter((s) => s.status === "success" || s.status === "error")
                    .length
                }{" "}
                of {weekApplyStatuses.length} week{weekApplyStatuses.length !== 1 ? "s" : ""}{" "}
                complete
              </p>
            </div>
          </div>
        ...
        {successData ? (
          ...
          >
            <div
              className={`rounded-full p-6 ${successData.weeksApplied === 0 ? "bg-red-100 dark:bg-red-900/30" : successData.weeksFailed > 0 ? "bg-amber-100 dark:bg-amber-900/30" : "bg-green-100 dark:bg-green-900/30"}`}
            >
            ...
            <div className="text-center space-y-2">
              ...
              {successData.creditsUsed > 0 && (
                <p
                  ...
                  data-testid="text-success-credits-used"
                >
                  {successData.creditsUsed} credit{successData.creditsUsed !== 1 ? "s" : ""} used ·{" "}
                  {successData.creditsRemaining} remaining this month
                </p>
              )}
            </div>
            {/* Per-week result breakdown */}
            {(successData.weekResults.length > 1 ||
              successData.weekResults.some((w) => w.status === "error")) && (
              <div className="w-full max-w-sm space-y-1.5" data-testid="section-week-results">
                {successData.weekResults.map((ws) => (
                  <div
                    key={ws.weekStart}
                    className="flex items-center gap-3 rounded-lg border px-3 py-2"
                    data-testid={`row-week-result-${ws.weekStart}`}
                  >
                    ...
                    <span
                      className={`text-xs font-medium shrink-0 ${ws.status === "success" ? "text-green-600 dark:text-green-400" : "text-destructive"}`}
                    >
        ...
        !multiWeekResult ? (
          <div className="space-y-6 py-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Keep stops on their zone-assigned day
                  </p>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Allow stops to be scheduled on Saturday
                  </p>
                </div>
                <Switch
            ...
            {geocodeError && (
              <div
                className="rounded-lg border border-orange-300 bg-orange-50 dark:border-orange-700 dark:bg-orange-950/30 p-4 space-y-2"
                data-testid="banner-weekly-geocode-error"
              >
                <div className="flex items-start gap-2">
                  <AlertTriangle className="h-4 w-4 text-orange-600 dark:text-orange-400 shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-orange-800 dark:text-orange-200">
                      Not enough geocoded stops
                    </p>
            ...
            <ScrollArea className="flex-1 min-h-0">
              {activeWeek ? (
                <div
                  ...
                  data-testid={`panel-week-${activeWeek.weekNum}`}
                >
                  {/* Week accept toggle */}
                  {!activeWeek.empty && (
                    <div className="flex items-center justify-between p-3 border rounded-lg bg-muted/20">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {activeWeek.creditsRequired} credit
                        {activeWeek.creditsRequired !== 1 ? "s" : ""} ·{" "}
                        {activeWeek.proposed.totalStops} stops
                      </span>
                    </div>
                  )}
                  ...
                  {activeWeek.empty ? (
                    <div
                      ...
                      data-testid={`empty-week-${activeWeek.weekNum}`}
                    >
                    ...
                    <>
                      ...
                      <div
                        ...
                        data-testid={`summary-week-${activeWeek.weekNum}`}
                      >
                        <div className="flex items-center justify-between mb-3">
                          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                            Impact Summary — {activeWeek.weekLabel}
                          </p>
                          <span
                            className="text-xs text-muted-foreground"
                            data-testid={`text-improvement-week-${activeWeek.weekNum}`}
                          >
                            {activeWeek.improvementPct}% improvement
                          </span>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          <div
                            className="text-center"
                            data-testid={`metric-miles-week-${activeWeek.weekNum}`}
                          >
                            <p className="text-2xl font-bold text-primary">
                              {activeWeek.milesSaved}
                            </p>
                            <p className="text-xs font-medium mt-0.5">Miles Saved</p>
                          </div>
                          <div
                            className="text-center"
                            data-testid={`metric-time-week-${activeWeek.weekNum}`}
                          >
                            <p className="text-2xl font-bold text-primary">
                              {formatMinutes(activeWeek.minutesSaved)}
                            </p>
                            <p className="text-xs font-medium mt-0.5">Time Saved</p>
                          </div>
                          <div
                            className="text-center"
                            data-testid={`metric-moves-week-${activeWeek.weekNum}`}
                          >
                          ...
                          <div
                            className="text-center"
                            data-testid={`metric-credits-week-${activeWeek.weekNum}`}
                          >
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-primary/10 text-sm">
                          <div className="p-2 rounded bg-muted/50">
                            ...
                            <p className="font-semibold">
                              {activeWeek.current.totalMiles} mi /{" "}
                              {formatMinutes(activeWeek.current.totalMinutes)}
                            </p>
                            <p className="text-[11px] text-muted-foreground">
                              {activeWeek.current.totalStops} stops
                            </p>
                          </div>
                          <div className="p-2 rounded bg-primary/10">
                            ...
                            <p className="font-semibold text-primary">
                              {activeWeek.proposed.totalMiles} mi /{" "}
                              {formatMinutes(activeWeek.proposed.totalMinutes)}
                            </p>
                            <p className="text-[11px] text-muted-foreground">
                              {activeWeek.proposed.totalStops} stops
                            </p>
                      ...
                      {/* Trust signals */}
                      <div
                        className="rounded-lg border bg-muted/30 px-4 py-3 flex flex-wrap gap-2 items-center"
                        data-testid={`section-trust-week-${activeWeek.weekNum}`}
                      >
                        <Shield className="h-4 w-4 text-muted-foreground shrink-0" />
                        <Badge variant="outline" className="text-xs font-medium">
                          {activeWeek.movedStops.length} jobs changed day
                        </Badge>
                        <Badge variant="outline" className="text-xs font-medium">
                        ...
                        <Badge variant="outline" className="text-xs font-medium">
                          {activeWeek.techCount} technician{activeWeek.techCount !== 1 ? "s" : ""}
                        </Badge>
                        {(activeWeek.excludedWeekendCount ?? 0) > 0 && (
                          <Badge variant="outline" className="text-xs font-medium">
                            {activeWeek.excludedWeekendCount} Saturday stops kept
                          </Badge>
                        )}
                      </div>
                      ...
                      {activeWeek.ungeocodedStops && activeWeek.ungeocodedStops.length > 0 && (
                        <div
                          className="rounded-lg border border-orange-300 bg-orange-50 dark:border-orange-700 dark:bg-orange-950/30 p-4 space-y-2"
                          data-testid={`banner-geocode-week-${activeWeek.weekNum}`}
                        >
                          <div className="flex items-start gap-2">
                            <AlertTriangle className="h-4 w-4 text-orange-600 dark:text-orange-400 shrink-0 mt-0.5" />
                            <div>
                              <p className="text-sm font-semibold text-orange-800 dark:text-orange-200">
                                {activeWeek.ungeocodedStops.length} stop
                                {activeWeek.ungeocodedStops.length !== 1 ? "s" : ""} excluded —
                                missing coordinates
                              </p>
                              <p className="text-xs text-orange-700 dark:text-orange-300 mt-0.5">
                                Fix their addresses and re-analyze to include them.
                              </p>
                            </div>
                          </div>
                          <ul className="space-y-0.5 ml-6">
                            {activeWeek.ungeocodedStops.map((stop) => (
                              <li
                                ...
                                className="flex items-start gap-1.5 text-xs text-orange-800 dark:text-orange-200"
                              >
                      ...
                      <div
                        className="space-y-3"
                        data-testid={`panel-day-by-day-week-${activeWeek.weekNum}`}
                      >
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                          Day by Day Routes
                        </p>
                        {activeWeek.proposed.days
                          .filter((d) => d.totalStops > 0)
                          .map((day) => (
                            <Card
                              key={day.day}
                              data-testid={`day-card-${activeWeek.weekNum}-${day.day}`}
                            >
                                    ...
                                    <div key={rIdx} className="space-y-1.5">
                                      <div className="flex items-center justify-between gap-2">
                                        <p className="text-xs font-medium flex items-center gap-1.5 min-w-0 flex-wrap">
                                          ...
                                          {(() => {
                                            const tech = techId
                                              ? teamMembers.find((m) => m.id === techId)
                                              : null;
                                            if (tech) {
                                              return (
                                                <Badge
                                      ...
                                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground pt-1">
                                        ...
                                        <span>
                                          {route.estimatedMiles} mi · {route.estimatedMinutes} min ·{" "}
              ...
              >
                <span className="text-amber-700 dark:text-amber-400">
                  ...
                  <span className="font-semibold" data-testid="text-confirm-credits-cost">
                    {totalAcceptedCredits}
                  </span>{" "}
                  credit{totalAcceptedCredits !== 1 ? "s" : ""} across{" "}
                  <span className="font-semibold">{acceptedWeeks.size}</span> week
                  {acceptedWeeks.size !== 1 ? "s" : ""}. You currently have{" "}
                  <span className="font-semibold" data-testid="text-confirm-credits-balance">
                    {liveCredits}
                  </span>{" "}
                  ...
                  <>
                    <span>
                      <span
                        ...
                        data-testid="text-credits-required"
                      >
                        {totalAcceptedCredits}
                      </span>{" "}
                      credit{totalAcceptedCredits !== 1 ? "s" : ""} across {acceptedWeeks.size} week
                      {acceptedWeeks.size !== 1 ? "s" : ""} · {credits} available
                    </span>
                ...
                >
                  {applyMutation.isPending ? (
                    ...
                    <>
                      ...
                      Confirm — {totalAcceptedCredits} credit{totalAcceptedCredits !== 1 ? "s" : ""}
                    </>
                  ) : (
                    <>
