...
export default function CrmDashboard() {
  ...
  const tiles = [
    ...
    },
    {
      ...
      value: `$${((stats?.pipelineValue ?? 0) / 100).toLocaleString()}`,
      icon: BarChart3,
      href: "/crm/pipeline",
      color: "text-primary",
    },
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight" data-testid="text-crm-dashboard-title">
          CRM Overview
        </h1>
        <p className="text-muted-foreground mt-1 text-sm">
          Sales pipeline, contacts, and customer data at a glance.
        </p>
      </div>
        ...
        {tiles.map((tile) => {
          ...
          return (
            <Link
              key={tile.label}
              href={tile.href}
              data-testid={`card-crm-stat-${tile.label.toLowerCase().replace(/ /g, "-")}`}
            >
          ...
          {quickLinks.map((link) => (
            <Link key={link.href} href={link.href}>
              <Button
                variant="outline"
                size="sm"
                data-testid={`link-crm-quick-${link.label.toLowerCase().replace(/ /g, "-")}`}
              >
                {link.label}
              </Button>
            </Link>
