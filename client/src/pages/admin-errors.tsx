        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span
                className={`text-[10px] border px-1.5 py-0.5 rounded font-medium ${STATUS_COLORS[report.status] || ""}`}
              >
                {report.status}
              </span>
              {report.fixTask && (
                <span
                  className={`text-[10px] border px-1.5 py-0.5 rounded font-medium ${report.fixTask.status === "done" ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-purple-500/20 text-purple-400 border-purple-500/30"}`}
                >
              ...
              {report.pageUrl && (
                <span className="text-[10px] text-[#858585] truncate max-w-[180px]">
                  {report.pageUrl}
                </span>
              )}
            </div>
...
export default function AdminErrors() {
  ...
  const { data: allReports = [], isLoading: allLoading } = useQuery<ErrorReport[]>({
    ...
    queryFn: adminFetchFn(
      buildQueryUrl(statusFilter, severityFilter, fromDate, toDate, page, false)
    ),
  ...
  const { data: groupedReports = [], isLoading: groupedLoading } = useQuery<GroupedErrorReport[]>({
    queryKey: [
      "/api/admin/error-reports/grouped",
      statusFilter,
      severityFilter,
      fromDate,
      toDate,
    ...
    queryFn: adminFetchFn(
      buildQueryUrl(statusFilter, severityFilter, fromDate, toDate, page, true)
    ),
  ...
  const hasNextPage =
    viewMode === "all" ? allReports.length === PAGE_SIZE : groupedReports.length === PAGE_SIZE;

  const selected = selectedId ? (allReports.find((r) => r.id === selectedId) ?? null) : null;
  ...
  const bulkStatusMutation = useMutation({
    mutationFn: async ({ message, status }: { message: string; status: string }) => {
      const res = await adminRequest("POST", "/api/admin/error-reports/bulk-status", {
        message,
        status,
      });
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/admin/error-reports/stats"] });
      toast({
        title: `${data.updated} error${data.updated !== 1 ? "s" : ""} marked as ${variables.status}`,
      });
    },
  ...
  const markFixTaskDoneMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: "open" | "done" }) => {
      const res = await adminRequest("PATCH", `/api/admin/error-reports/${id}/fix-task`, {
        status,
      });
  ...
  return (
    <div
      className="flex h-full bg-[#1e1e1e] text-[#d4d4d4] font-mono"
      data-testid="admin-errors-page"
    >
      <div
        className={`flex flex-col ${detailReport ? "w-1/2" : "w-full"} transition-all duration-200`}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#3c3c3c] flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            {openCount > 0 && (
              <span
                className="text-xs bg-red-500/20 text-red-400 border border-red-500/30 px-2 py-0.5 rounded-full"
                data-testid="badge-open-count"
              >
                {openCount} open
              </span>
            )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div
              className="flex items-center border border-[#3c3c3c] rounded overflow-hidden"
            ...
            <Select value={severityFilter} onValueChange={handleFilterChange(setSeverityFilter)}>
              <SelectTrigger
                className="h-7 w-[130px] text-xs bg-[#252526] border-[#3c3c3c] text-[#d4d4d4]"
            ...
            <Select value={statusFilter} onValueChange={handleFilterChange(setStatusFilter)}>
              <SelectTrigger
                className="h-7 w-[140px] text-xs bg-[#252526] border-[#3c3c3c] text-[#d4d4d4]"
        ...
        {isLoading ? (
          ...
          <div
            className="flex flex-col items-center justify-center h-64 text-[#858585]"
            data-testid="empty-errors"
          >
          ...
          <>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        {report.severity && (
                          <span
                            className={`text-[10px] border px-1.5 py-0.5 rounded font-semibold ${SEVERITY_COLORS[report.severity] || ""}`}
                        ...
                        <span
                          className={`text-[10px] border px-1.5 py-0.5 rounded font-medium ${TYPE_COLORS[report.errorType] || ""}`}
                      ...
                      <p
                        ...
                        data-testid={`text-message-${report.id}`}
                      >
                        {report.message}
                      </p>
              ...
              {viewMode === "grouped" &&
                groupedReports.map((group) => {
                  ...
                  const isBulkPending =
                    ...
                    bulkStatusMutation.variables?.message === group.message;
                  return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            {group.severity && (
                              <span
                                className={`text-[10px] border px-1.5 py-0.5 rounded font-semibold ${SEVERITY_COLORS[group.severity] || ""}`}
                                data-testid={`badge-severity-${group.latestId}`}
                              >
                                {SEVERITY_ICONS[group.severity]} {group.severity.toUpperCase()}
                              </span>
                            )}
                            <span
                              className={`text-[10px] border px-1.5 py-0.5 rounded font-medium ${TYPE_COLORS[group.errorType] || ""}`}
                            >
                              {group.errorType.toUpperCase()}
                            </span>
                            <span
                              className={`text-[10px] border px-1.5 py-0.5 rounded font-medium ${STATUS_COLORS[group.status] || ""}`}
                            >
                              {group.status}
                            </span>
                            <span
                            ...
                            <span className="text-[10px] text-[#858585]">
                              last {formatTs(group.lastSeen)}
                            </span>
                          </div>
                          <p
                            className="text-sm text-[#e8e8e8] truncate"
                            data-testid={`text-group-message-${group.latestId}`}
                          >
                            {group.message}
                          </p>
                        ...
                        >
                          {group.status !== "acknowledged" && (
                            ...
                            >
                              {isBulkPending &&
                              bulkStatusMutation.variables?.status === "acknowledged" ? (
                                <Loader2 className="h-3 w-3 animate-spin" />
                              ) : (
                          ...
                          {group.status !== "resolved" && (
                            ...
                            >
                              {isBulkPending &&
                              bulkStatusMutation.variables?.status === "resolved" ? (
                                <Loader2 className="h-3 w-3 animate-spin" />
                              ) : (
            ...
            <div
              className="flex items-center justify-between px-5 py-3 border-t border-[#3c3c3c] text-xs text-[#858585]"
            ...
            >
              ...
              <span data-testid="text-page-info">
                Page {page + 1} ·{" "}
                {viewMode === "grouped" ? groupedReports.length : allReports.length} records
              </span>
              <Button
                size="sm"
                variant="ghost"
      ...
      {detailReport && (
        <div
          className="w-1/2 border-l border-[#3c3c3c] flex flex-col overflow-auto"
          ...
          <div className="p-5 space-y-5">
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ...
                {
                  label: "Severity",
                  value: detailReport.severity
                    ? `${SEVERITY_ICONS[detailReport.severity]} ${detailReport.severity.toUpperCase()}`
                    : "—",
                },
                ...
                <div
                  key={label}
                  className={`bg-[#252526] rounded p-2 ${
                    label === "Severity" && detailReport.severity
                      ? SEVERITY_COLORS[detailReport.severity]
                          .replace(/text-\S+/, "")
                          .replace(/border-\S+/, "")
                          .trim()
                      : ""
                ...
                >
                  ...
                  <div
                    className={`break-all font-medium ${label === "Severity" && detailReport.severity ? SEVERITY_COLORS[detailReport.severity].split(" ").find((c) => c.startsWith("text-")) || "text-[#d4d4d4]" : "text-[#d4d4d4]"}`}
                  >
                    {value}
                  </div>
                </div>
            ...
            {detailReport.pageUrl && (
              <div>
                ...
                <div
                  className="bg-[#252526] rounded p-2 text-xs text-[#9cdcfe] break-all"
                  data-testid="detail-page-url"
                >
                  {detailReport.pageUrl}
                </div>
              </div>
            ...
            <div>
              ...
              <div
                className="bg-[#252526] rounded p-3 text-sm text-[#f44747] break-words"
            ...
            {detailReport.stack && (
              <div>
                ...
                <pre
                  className="bg-[#0d0d0d] rounded p-3 text-[11px] text-[#ce9178] overflow-auto max-h-64 whitespace-pre-wrap break-words"
                  data-testid="detail-stack"
                >
                  {detailReport.stack}
                </pre>
              </div>
              ...
              {detailReport.fixTask ? (
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[10px] text-[#858585]" data-testid="fix-task-date">
                          Created {formatTs(detailReport.fixTask.createdAt)}
                        </span>
                      </div>
                    </div>
                    {detailReport.fixTask.status === "open" ? (
                      <Button
                        ...
                        onClick={() =>
                          markFixTaskDoneMutation.mutate({ id: detailReport.id, status: "done" })
                        }
                        data-testid="button-mark-fix-done"
                      >
                        {markFixTaskDoneMutation.isPending ? (
                      ...
                      <Button
                        ...
                        disabled={markFixTaskDoneMutation.isPending}
                        onClick={() =>
                          markFixTaskDoneMutation.mutate({ id: detailReport.id, status: "open" })
                        }
                ...
                >
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter")
                        fixTaskMutation.mutate({ id: detailReport.id, title: fixTaskTitle });
                      if (e.key === "Escape") {
                        setShowFixTaskForm(false);
                        setFixTaskTitle("");
                      }
            ...
            <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-[#3c3c3c]">
              {detailReport.status !== "acknowledged" && (
                <Button
                  ...
                  onClick={() =>
                    updateMutation.mutate({ id: detailReport.id, status: "acknowledged" })
                  }
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold text-foreground">
                {bulkConfirm?.count ?? 0} error{(bulkConfirm?.count ?? 0) !== 1 ? "s" : ""}
              </span>{" "}
              as <span className="font-semibold text-foreground">{bulkConfirm?.status}</span>. This
