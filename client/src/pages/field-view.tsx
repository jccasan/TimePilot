...
import {
  RefreshCw, MapPin, CheckCircle, Clock, User, AlertCircle, Info,
...
export default function FieldView() {
  ...
  const { data: visits, isLoading, isFetching, refetch } = useQuery<EnrichedVisit[]>({
  ...
  const routeGroups = useMemo<RouteGroup[]>(() => {
    ...
    groupMap.forEach((groupVisits, key) => {
      ...
      const tech = routeRecord?.technicianId ? team?.find(t => t.id === routeRecord.technicianId) : null;
      const techName = tech ? `${tech.firstName} ${tech.lastName}` : null;

      const completedCount = sorted.filter(v =>
      ...
      const nextStop = sorted.find(v =>
        (v.status === "scheduled" || v.status === "in_progress") && !overdueIds.has(v.id)
      ) ?? null;
  ...
  const validVisits = useMemo(() => {
    ...
    return visits.filter(v =>
      v.property?.latitude != null && v.property?.longitude != null &&
      !isNaN(v.property.latitude) && !isNaN(v.property.longitude)
    );
  }, [visits]);

  const nextStopIds = useMemo(() => new Set(routeGroups.map(g => g.nextStop?.id).filter(Boolean) as string[]), [routeGroups]);
  ...
  const completedStops = visits?.filter(v =>
  ...
  return (
    <div className="flex h-full overflow-hidden" data-testid="field-view-page">
      {sidebarOpen && (
        <div className="w-72 shrink-0 border-r flex flex-col bg-background" data-testid="field-view-sidebar">
          <div className="p-3 border-b flex items-center justify-between gap-2">
            <div>
              <h1 className="font-semibold text-base" data-testid="field-view-title">Live Field Map</h1>
          ...
          <div className="p-3 border-b">
            ...
            <div className="grid grid-cols-2 gap-1.5 text-center">
              ...
              <div className="rounded-md bg-green-50 dark:bg-green-950/30 p-1.5" data-testid="stat-completed-stops">
                <p className="text-base font-bold text-green-700 dark:text-green-400">{completedStops}</p>
                <p className="text-[10px] text-muted-foreground">Done</p>
              </div>
              <div className="rounded-md bg-blue-50 dark:bg-blue-950/30 p-1.5" data-testid="stat-inprogress-stops">
                <p className="text-base font-bold text-blue-700 dark:text-blue-400">{inProgressStops}</p>
                <p className="text-[10px] text-muted-foreground">Active</p>
              </div>
              <div className="rounded-md bg-red-50 dark:bg-red-950/30 p-1.5" data-testid="stat-overdue-stops">
          ...
          <div className="p-3 pb-1 border-b">
            <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider mb-2">Legend</p>
            <div className="space-y-1" data-testid="map-legend">
              <div className="flex items-center gap-2 text-xs">
                <span className="w-3.5 h-3.5 rounded-full shrink-0" style={{ background: "#22c55e" }} />
                <span>Next stop</span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <span className="w-3.5 h-3.5 rounded-full shrink-0" style={{ background: "#3b82f6" }} />
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className="w-3.5 h-3.5 rounded-full shrink-0" style={{ background: "#9ca3af" }} />
          ...
          <ScrollArea className="flex-1">
            <div className="p-2 space-y-2">
              {isLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground text-sm" data-testid="no-routes-message">
                  <MapPin className="h-6 w-6 mx-auto mb-2 opacity-40" />
                  No visits scheduled for this date.
                </div>
              ) : (
                routeGroups.map(group => (
                  <Card key={group.routeId ?? "unassigned"} className="p-0" data-testid={`route-card-${group.routeId ?? "unassigned"}`}>
                    <CardContent className="p-2.5 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm truncate" data-testid={`route-name-${group.routeId}`}>
                            {group.routeName}
                          </p>
                          {group.techName && (
                            <p className="text-xs text-muted-foreground flex items-center gap-1 truncate" data-testid={`route-tech-${group.routeId}`}>
                        ...
                        <div className="flex flex-col items-end gap-0.5 shrink-0">
                          <Badge variant="outline" className="text-[10px]" data-testid={`route-progress-${group.routeId}`}>
                            {group.completedCount}/{group.totalCount}
                          </Badge>
                          {group.overdueIds.size > 0 && (
                            <Badge variant="destructive" className="text-[10px]" data-testid={`route-overdue-${group.routeId}`}>
                      ...
                      <div className="w-full bg-muted rounded-full h-1.5 overflow-hidden" data-testid={`progress-bar-${group.routeId}`}>
                        <div
                      ...
                      {group.nextStop && (
                        <div className="text-xs space-y-0.5" data-testid={`next-stop-${group.routeId}`}>
                      ...
                      {!group.nextStop && group.totalCount > 0 && (
                        <div className="text-xs flex items-center gap-1 text-green-600 dark:text-green-400" data-testid={`all-done-${group.routeId}`}>
      ...
      <div className="flex-1 relative min-w-0">
        ...
        {(tokenLoading || isLoading) && (
          <div className="absolute inset-0 flex items-center justify-center bg-background z-10" data-testid="map-loading-overlay">
        ...
        {!tokenLoading && !tokenData?.token && (
          <div className="absolute inset-0 flex items-center justify-center bg-background z-10" data-testid="map-no-token">
            <div className="text-center text-muted-foreground max-w-xs px-6">
              <AlertCircle className="h-8 w-8 mx-auto mb-3" />
              <p className="text-sm">Map unavailable. Configure <code className="text-xs bg-muted px-1 py-0.5 rounded">MAPBOX_PUBLIC_TOKEN</code> to enable the live field map.</p>
            </div>
          </div>
        )}

        {!tokenLoading && tokenData?.token && validVisits.length === 0 && !isLoading && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none" data-testid="map-no-coords-notice">
