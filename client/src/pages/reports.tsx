...
const periodOptions = [
  ...
  { group: "Annual", items: [{ value: "annual", label: "Full Year" }] },
  {
    group: "Projections",
    items: [
...
function ReportsContent() {
  ...
  const totalPeriodRevenue = data
    ? data.monthlyRevenue.filter((m) => !m.projected).reduce((sum, m) => sum + m.revenue, 0)
    : 0;
  const totalProjectedRevenue = data
    ? data.monthlyRevenue.filter((m) => m.projected).reduce((sum, m) => sum + m.revenue, 0)
    : 0;
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-2xl font-bold" data-testid="text-total-collected">
                $
                {(data?.totalCollected ?? 0).toLocaleString(undefined, {
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-2xl font-bold" data-testid="text-total-outstanding">
                $
                {(data?.totalOutstanding ?? 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </div>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-2xl font-bold text-primary" data-testid="text-booked-revenue">
                $
                {(data?.bookedRevenue ?? 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </div>
                ...
                <div className="text-right">
                  ...
                  {hasProjections && (
                    <p className="text-xs text-muted-foreground">
                      +$
                      {totalProjectedRevenue.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}{" "}
                      projected
                  ...
                  <div key={m.month} className="space-y-1">
                    <div className="flex items-center justify-between gap-2 text-sm">
                      <div className="flex items-center gap-1.5 min-w-[130px]">
                        <span
                          className={
                            m.projected ? "text-muted-foreground italic" : "text-muted-foreground"
                          }
                        >
                          {m.month}
                        </span>
                      ...
                      <span className={`font-medium ${m.projected ? "text-muted-foreground" : ""}`}>
                        $
                        {m.revenue.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </span>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4" data-testid="chart-visits">
                ...
                <div className="space-y-2">
                  ...
                  <Progress
                    value={
                      data?.thisMonthVisits.total
                        ? (data.thisMonthVisits.completed / data.thisMonthVisits.total) * 100
                        : 0
                    }
                    className="h-2"
                  />
