...
- **Guided Onboarding Wizard**: First-time user onboarding modal (`client/src/components/onboarding-wizard.tsx`) that shows a 7-step tour to new users: Welcome, Clients, Routes, Scheduling, Invoicing, Pricing Tools, and a completion step with CTAs to Dashboard or Add First Client. Uses localStorage (`onboarding_wizard_seen_${userId}`) to track if seen. Suppresses Rover chatbot intro for new users. Rendered globally in `App.tsx`. Features step dots, progress bar, back/next/skip controls.
