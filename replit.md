...
- **Financials**: Detailed invoicing with Stripe integration, payment ledger, cost-based automatic price calculator, and a profitability dashboard (per customer and route). Revenue calculations (`getRevenueForPeriod`) use `invoices.paidAt` (not `createdAt`) for date-range filtering, with PostgreSQL `AT TIME ZONE` double-conversion (`timestamp AT TIME ZONE 'UTC' AT TIME ZONE <company_tz>`) to correctly bucket revenue by company-local dates.
...
- **Dashboard Widgets**: Customizable drag-and-drop dashboard using react-grid-layout. Widgets include MRR, Revenue This Month, Requires Invoicing, Overdue Invoices, Today's Visits, Active Clients, Jobs, Team Size, SMS Sent, Emails Sent, Quick Actions, Recent Activity, Upcoming Visits, Revenue Chart (6-month bar chart), Route Summary, Quick Notes (server-persisted via `companies.dashboardNotes`), Current Plan, Weather Forecast (5-day via Open-Meteo API), and Route Map Preview (Mapbox GL mini-map of today's routes with color-coded markers and route lines). Widget library drawer lets users add/remove widgets and reset to default layout. Layout persisted as JSONB layout objects in `companies.dashboardLayout`. Backward-compatible with legacy `string[]` format. Empty array layouts supported. Debounced save prevents write churn. Mobile view stacks widgets in single column with drag/resize disabled. API endpoints: `GET /api/company/revenue-chart`, `GET /api/company/recent-activity`, `GET /api/company/upcoming-visits`, `GET /api/company/weather`, `GET /api/company/route-map-data`.
