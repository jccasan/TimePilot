...
- **Import Wizard (v2)**: Full 7-step wizard wrapping the CSV import pipeline (`client/src/components/import-wizard.tsx`). Steps: 0=MigrationChecklist (6 pre-flight questions including "knowsNextServiceDate" scheduling anchor), 1=competitor-aware UploadScreen (Sweep & Go / Jobber / Google Sheets / Other with CSV drop-zone, URL fetch, or paste modes), 2=AI field mapping, 3=row review, 4=RouteAssignmentStep (preserve/rebuild/hybrid selector; routePreference wired through to the import runner to control how serviceDay is treated for each staged row), 5=RouteDayPreviewStep (day-distribution table from preview rows, Mon–Fri + Unassigned, with mode-specific note), 6=Final confirmation summary + "Stage Import" button that calls POST /api/imports/apply and navigates to /import/:batchId/resolve. Includes TwoLayerProgress indicator (Layer 1: Contact Info steps 0-3; Layer 2: Service Logic steps 4-6). Migration page Transfer tab (`client/src/pages/migration-page.tsx`) replaced old competitor detect flow with a 4-platform card selector that launches the wizard with `initialPlatform` pre-set. Competitor-specific parsing (Sweep & Go, Jobber) is handled server-side via `competitor-import.ts` and `sweepandgo-parser.ts` when platform is detected. Backend: `GET /api/imports/fetch-url` proxy endpoint for Google Sheets CSV export URLs (domain allowlisted, 15s timeout); `POST /api/imports/apply` now accepts `routePreference` and `platform` fields and passes them to `enqueueStagedCsvContactsImport` in `import-runner.ts`.
