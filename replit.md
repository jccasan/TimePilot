...
- **Jobs & Scheduling (Unified)**: Jobs are managed directly from the Scheduling page — there is no separate Jobs page (the `/jobs` route redirects to `/scheduling`). The Add Job dialog on the Scheduling page supports all fields: job type (one-off/recurring), customer, property, services, frequency, day of week, start/end time, anytime toggle, end conditions, assigned team member, and visit instructions. Creating a job via `POST /api/jobs` automatically generates visits for 6 months (no separate "Create Visits" step needed). Service plans support a `discount` field (percentage) and "Stop Only" flags for non-revenue visits. **Visit Edit Form**: Owner/admin users can edit individual visits (scheduled date, route assignment, technician notes) via an inline edit form in the Visit Details sheet. The sheet component uses `useQuery` on `/api/auth/user` directly to determine role-based edit access. Edits use `PATCH /api/visits/:id`. **Client Filter**: Search input filters visits by client name. **Hidden Status Toggle**: Cancelled/skipped visits hidden by default with a toggle switch showing count.
