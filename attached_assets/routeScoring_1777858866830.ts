...
import type { PlannedRoute, RoutePlannerSettings } from "./types.js";
...
export function scoreProfitability(route: PlannedRoute, settings: RoutePlannerSettings): number {
  ...
  const marginScore = clamp01(route.profitMargin / Math.max(0.01, settings.targetRouteProfitMargin));
...
export function scoreStability(
  ...
  const moved = route.assignedStops.filter((s) => context.movedCustomerIds!.has(s.customerId)).length;
...
export function calculateRouteScore(
  ...
  const hardWarnings = route.warnings.filter((w) =>
