...
import type { PlannedRoute, RoutePlannerSettings, RouteStop } from "./types.js";
...
export function calculateRouteEconomics(
  route: Pick<
    PlannedRoute,
    "assignedStops" | "totalServiceMinutes" | "totalDriveMinutes" | "totalRouteMinutes" | "estimatedMiles"
  >,
  settings: RoutePlannerSettings,
): RouteEconomics {
  ...
  const revenuePerRouteHour = route.totalRouteMinutes > 0 ? revenue / (route.totalRouteMinutes / 60) : 0;
  const revenuePerMile = route.estimatedMiles > 0 ? revenue / route.estimatedMiles : undefined;
  const stopsPerMile = route.estimatedMiles > 0 ? route.assignedStops.length / route.estimatedMiles : undefined;
...
export function deriveRouteWarningsAndRecommendations(
  ...
  if (route.totalRouteMinutes > settings.hardMaxRouteMinutes) {
    if (settings.allowTimeOverride) {
      const cap =
        settings.hardMaxRouteMinutes + (settings.maxAllowedTimeOverrideMinutes ?? 0);
