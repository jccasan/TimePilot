...
import { clusterStopsByGeography } from "./clustering.js";
import {
  buildPlanningWindow,
...
import { getDueCustomers, splitRoutableCustomers } from "./dateFilters.js";
import { approximateDriveMinutes, haversineMiles, type LatLng } from "./geoUtils.js";
import {
  annotateStopsWithLegCosts,
  buildPlannedRouteFromBreakdown,
  calculateRouteDuration,
} from "./routeDuration.js";
import {
  calculateRouteEconomics,
  calculateStopLevelProfitability,
  deriveRouteWarningsAndRecommendations,
} from "./routeEconomics.js";
import { calculateRouteScore } from "./routeScoring.js";
import {
  computeStabilityContext,
} from "./routeStability.js";
import type { RoutingProvider } from "./routingProvider.js";
import type { TravelLegCacheService } from "./travelLegCache.js";
import type {
  Customer,
  ExcludedCustomer,
  FourWeekPlanResult,
...
async function mergeUnderutilizedRoutes(
  ...
  for (const small of sorted) {
    ...
    if (small.totalRouteMinutes >= settings.targetMinRouteMinutes) {
      merged.push(small);
      continue;
    }
...
function summarizeWeek(
  ...
  const avgMargin =
    routes.length > 0
      ? routes.reduce((s, r) => s + r.profitMargin, 0) / routes.length
      : 0;
  return {
...
export async function assignNewCustomerToBestRoute(
  ...
  const candidates = new Map<string, { route: PlannedRoute; nearestStopId: string; nearestDist: number }>();
  ...
  for (const cand of topCandidates) {
    ...
    for (let idx = 0; idx <= cand.route.assignedStops.length; idx++) {
      ...
      if (
        trial.totalRouteMinutes > settings.hardMaxRouteMinutes &&
        !settings.allowTimeOverride
      ) {
        continue;
      }
    ...
    const candidateResult: NewCustomerAssignmentResult & { _routeId: string; _trial: PlannedRoute } = {
  ...
  if (!bestPick) {
    return {
      ...
      addedRevenue: newCustomer.revenuePerVisit,
      profitImpact: 0,
      warnings: ["no_candidate_route_could_absorb_customer_within_time_limit"],
      needsManualRouteAssignment: true,
      candidateRoutesReviewed: topCandidates.length,
    };
  }

  if (bestPick.profitImpact < 0) {
    warnings.push("insertion_reduces_route_profit");
  }
  if (bestPick.addedDriveMinutes > 25) {
    warnings.push("insertion_adds_significant_drive_time");
  }

  // Reference unused estimator (keep import) — silences linter when geo helpers
  // are needed only by the trial path.
  void approximateDriveMinutes;
