...
import type { Customer, RoutePlannerSettings } from "./types.js";
...
export function clusterStopsByGeography(
  ...
  const softTimeBudget =
    settings.targetMaxRouteMinutes - settings.routeBufferMinutes - 30;
  ...
  while (points.some((p) => !p.used)) {
    ...
    while (true) {
      ...
      const projected = runningMinutes + driveAdd + cand.serviceMinutes + settings.perStopBufferMinutes;
