...
async function seed() {
  ...
  const insertedContacts = await db
    .insert(contacts)
    .values(contactsData.map((c) => ({ ...c, companyId })))
    .returning();
  ...
  const propertiesData = [
    ...
    },
    {
      ...
      numberOfDogs: 1,
      yardSize: "large",
      specialInstructions: "Dogs are in backyard",
      latitude: "30.2550",
      longitude: "-97.7260",
    },
    {
  ...
  const servicePlansData = [
    ...
    },
    {
      companyId,
      contactId: mike.id,
      propertyId: mikeProp.id,
      frequency: "biweekly" as const,
      dayOfWeek: "tuesday" as const,
      pricePerVisit: "20.00",
      isActive: true,
      startDate,
      routeId: southRoute.id,
    },
    {
      companyId,
      contactId: james.id,
      propertyId: jamesProp.id,
      frequency: "weekly" as const,
      dayOfWeek: "wednesday" as const,
      pricePerVisit: "18.00",
      isActive: true,
      startDate,
      routeId: eastRoute.id,
    },
    {
      companyId,
      contactId: david.id,
      propertyId: davidProp.id,
      frequency: "monthly" as const,
      dayOfWeek: "thursday" as const,
      pricePerVisit: "25.00",
      isActive: true,
      startDate,
      routeId: westRoute.id,
    },
  ];
  ...
  for (const plan of insertedPlans) {
    ...
    while (
      isBefore(current, rangeEnd) ||
      format(current, "yyyy-MM-dd") === format(rangeEnd, "yyyy-MM-dd")
    ) {
      const dateStr = format(current, "yyyy-MM-dd");
      const isPast =
        isBefore(current, today) && format(current, "yyyy-MM-dd") !== format(today, "yyyy-MM-dd");

      if (plan.frequency === "biweekly") {
        const weeksSinceStart = Math.floor(
          (current.getTime() - rangeStart.getTime()) / (7 * 24 * 60 * 60 * 1000)
        );
  ...
  await db.insert(invoiceLineItems).values([
    {
      invoiceId: inv1.id,
      description: "Pet waste removal - 123 Oak Street",
      quantity: 1,
      unitPrice: "15.00",
      total: "15.00",
    },
    ...
    {
      invoiceId: inv2.id,
      description: "Pet waste removal - 456 Elm Avenue",
      quantity: 1,
      unitPrice: "20.00",
      total: "20.00",
    },
    ...
    {
      invoiceId: inv3.id,
      description: "Pet waste removal - 321 Cedar Lane",
      quantity: 1,
      unitPrice: "18.00",
      total: "18.00",
    },
