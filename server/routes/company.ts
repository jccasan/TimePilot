...
import { upsertTechLocation, getTechLocationsForCompany } from "../services/tech-location-store";
...
export async function registerCompanyRoutes(app: Express): Promise<void> {
  ...
  app.post("/api/technician/location", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { companyId, userId } = await getCompanyContext(req);
  ...
  app.get("/api/technician/locations", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { companyId } = await getCompanyContext(req);
