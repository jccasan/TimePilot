...
import { roundCoord, type LatLng } from "./geoUtils";
import type { RoutingProfile, TravelLegCacheRecord } from "./types";
...
export interface TravelLegCacheService {
  ...
  invalidate(origin: LatLng, destination: LatLng, profile: RoutingProfile): Promise<void>;
...
export class InMemoryTravelLegCache implements TravelLegCacheService {
  ...
  async invalidate(origin: LatLng, destination: LatLng, profile: RoutingProfile): Promise<void> {
    const key = this.buildCacheKey(origin, destination, profile, this.precision);
    this.store.delete(key);
  }
