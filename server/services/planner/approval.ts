...
import type { FourWeekPlanResult, PlannedRoute, RoutePlan, RoutePlanSource } from "./types";
...
export async function approveRoutePlan(
  ...
  for (const week of next.planningWindow.weeks) {
    week.plannedRoutes = week.plannedRoutes.map(
      (r): PlannedRoute => ({ ...r, status: "approved" })
    );
  }
...
export async function rejectRoutePlan(
  ...
  for (const week of next.planningWindow.weeks) {
    week.plannedRoutes = week.plannedRoutes.map(
      (r): PlannedRoute => ({ ...r, status: "rejected" })
    );
  }
  await store.save(next);
  return next;
