...
export function buildReportEmailText(report: GeneratedReport): string {
  const lines: string[] = [
    `${report.companyName} — ${report.reportName}`,
    `Generated ${report.generatedAt.toLocaleDateString()} | Period: ${report.dateRange.start} to ${report.dateRange.end}`,
    "",
  ];
  ...
  lines.push("Log in to view full interactive reports.");
  return lines.join("\n");
}
