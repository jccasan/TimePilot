      ...
      const depositInvoice = await storage.createInvoiceWithLineItems(
        {
          ...
          notes: `New customer deposit for service starting ${servicePlan.startDate}`,
      ...
      sendInvoiceEmail(depositInvoice.id, companyId, { baseUrl })
        .then(async (emailResult) => {
          ...
          const sentVia = [emailSent ? "email" : null, smsSent ? "SMS" : null]
            .filter(Boolean)
            .join(" and ");
          const sentNote = sentVia ? ` Sent to customer via ${sentVia}.` : "";
  ...
  if (result) {
    ...
    storage
      .createNotification({
        companyId,
        type: "general",
        title: "Customer Onboarding — Balance Due",
    ...
    if (monthlyRate > 0) {
      ...
      storage
        .createNotification({
          companyId,
          type: "general",
          title: "Customer Onboarding — First Month Due",
      ...
      if (anyOutstandingBalance) {
        ...
        Promise.all(
          ...
          .then((results) => {
            ...
            const sentNote = anySent ? " Balance invoice sent to customer." : "";
            storage
              .createNotification({
                companyId,
                type: "general",
