 ...
 *   - Creates a deposit invoice and sets stage to deposit_pending.
...
async function _createBalanceInvoice(
  ...
  if (result) {
    await storage.updateInvoice(result.invoiceId, companyId, { isOnboardingInvoice: true });
    await storage.updateContact(contactId, companyId, {
    ...
    if (stage === "deposit_pending" && contact.depositInvoiceId === invoiceId) {
      ...
      storage
        .createNotification({
          ...
          message: `Deposit paid for ${contact.firstName} ${contact.lastName}. ${balanceCreated ? "Prorated balance invoice created — awaiting payment to arm autopay." : "Autopay will be armed on balance payment."}`,
          isRead: false,
