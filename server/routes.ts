  ...
  app.post("/api/invoices/consolidate", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        console.error("[consolidate] createInvoiceWithLineItems failed:", insertErr.message, insertErr.detail || "", JSON.stringify({ invoiceNumber, dueDateStr, subtotal, totalAmount, discType, discVal, lineItemCount: allLineItems.length }));
        throw insertErr;
      }
