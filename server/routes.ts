...
<body>
  ...
  // All routes are prefixed /api/connect/v2/.
  // Authentication uses the same isAuthenticated middleware as the rest of the app.
  // Thin-event and V1-subscription webhooks are on separate paths (no auth).
  ...
  const {
    ...
    createConnectedAccountPortalSession,
    parseV2ThinEvent,
    retrieveV2Event,
    constructV1WebhookEvent,
  } = await import("./services/stripe-connect-v2");

  // Helper: resolve companyId from an authenticated request.
  async function getCompanyIdFromReq(req: Request): Promise<string | null> {
  ...
  // POST /api/connect/v2/account
  // Creates a new V2 connected account and persists the mapping.
  app.post("/api/connect/v2/account", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });

      const { displayName, contactEmail, country } = req.body;
      if (!displayName || !contactEmail || !country) {
        return res.status(400).json({ error: "displayName, contactEmail, and country are required" });
      }

      // Check if the company already has a V2 account.
      const existing = await storage.getConnectedAccount(companyId);
      if (existing) {
        return res.status(409).json({ error: "A connected account already exists for this company", accountId: existing.stripeAccountId });
      }

      // Create the account via the V2 API.
      const stripeAccountId = await createV2ConnectedAccount({ displayName, contactEmail, country });

      // Persist the mapping between our company and the Stripe account ID.
      const account = await storage.createConnectedAccount({ companyId, stripeAccountId });

      res.json({ accountId: stripeAccountId, record: account });
    } catch (err: any) {
      console.error("[V2 Connect] createAccount error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // GET /api/connect/v2/account
  // Returns the connected account record and its live Stripe status.
  app.get("/api/connect/v2/account", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });

      const account = await storage.getConnectedAccount(companyId);
      if (!account) return res.json({ exists: false });

      // Fetch live status from the V2 API (not from DB) per task requirements.
      const status = await getV2AccountStatus(account.stripeAccountId);
      res.json({ exists: true, accountId: account.stripeAccountId, record: account, status });
    } catch (err: any) {
      console.error("[V2 Connect] getAccount error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // POST /api/connect/v2/account/onboard
  // Generates a V2 hosted onboarding link.
  app.post("/api/connect/v2/account/onboard", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });

      const account = await storage.getConnectedAccount(companyId);
      if (!account) return res.status(404).json({ error: "No connected account found. Create one first." });

      const origin = `${req.protocol}://${req.get("host")}`;
      const url = await createV2AccountLink({
        accountId: account.stripeAccountId,
        returnUrl: `${origin}/connect-v2`,
        refreshUrl: `${origin}/connect-v2`,
      });

      res.json({ url });
    } catch (err: any) {
      console.error("[V2 Connect] onboard error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // POST /api/connect/v2/products
  // Creates a product + price on the connected account.
  app.post("/api/connect/v2/products", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });
      ...
      if (!name || !priceCents) return res.status(400).json({ error: "name and priceCents are required" });

      const result = await createProductOnConnectedAccount({
      ...
      console.error("[V2 Connect] createProduct error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // GET /api/connect/v2/products
  // Lists products on the connected account (for dashboard).
  app.get("/api/connect/v2/products", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });
      ...
      const products = await listProductsOnConnectedAccount(account.stripeAccountId);
      res.json(products);
    } catch (err: any) {
      console.error("[V2 Connect] listProducts error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // GET /api/connect/v2/storefront/:accountId/products
  // Public endpoint: lists products on a connected account for the storefront.
  // NOTE: In production, replace the raw Stripe account ID in the URL with a
  // slug or opaque identifier to avoid exposing internal account IDs.
  app.get("/api/connect/v2/storefront/:accountId/products", async (req: Request, res: Response) => {
    try {
      const { accountId } = req.params;
      const products = await listProductsOnConnectedAccount(accountId);
      res.json(products);
    } catch (err: any) {
      console.error("[V2 Connect] storefrontProducts error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // POST /api/connect/v2/storefront/:accountId/checkout
  // Public endpoint: creates a Checkout session for a direct charge purchase.
  // The price amount and application fee are ALWAYS derived server-side from Stripe
  // (never from client-supplied data) to prevent fee tampering.
  app.post("/api/connect/v2/storefront/:accountId/checkout", async (req: Request, res: Response) => {
    try {
      const { accountId } = req.params;
      // Only priceId is accepted from the client; priceCents is ignored even if supplied.
      // The authoritative unit_amount is retrieved server-side from Stripe.
      ...
      const result = await createStorefrontCheckout({
        ...
        priceId,
        successUrl: `${origin}/store/${accountId}/success`,
        cancelUrl: `${origin}/store/${accountId}`,
        customerEmail,
      });
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.error("[V2 Connect] storefrontCheckout error:", msg);
      res.status(500).json({ error: msg });
    }
  });

  // POST /api/connect/v2/subscription/checkout
  // Creates a platform subscription Checkout session for the connected account.
  app.post("/api/connect/v2/subscription/checkout", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });
      ...
      const result = await createPlatformSubscriptionCheckout({
        connectedAccountId: account.stripeAccountId,
        successUrl: `${origin}/connect-v2?subscribed=1`,
        cancelUrl: `${origin}/connect-v2`,
      });

      res.json(result);
    } catch (err: any) {
      console.error("[V2 Connect] subscriptionCheckout error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // POST /api/connect/v2/subscription/portal
  // Opens the billing portal for the connected account to manage their platform subscription.
  app.post("/api/connect/v2/subscription/portal", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const companyId = await getCompanyIdFromReq(req);
      if (!companyId) return res.status(400).json({ error: "No company found for user" });
      ...
      const origin = `${req.protocol}://${req.get("host")}`;
      const url = await createConnectedAccountPortalSession({
        connectedAccountId: account.stripeAccountId,
        returnUrl: `${origin}/connect-v2`,
      });
      ...
      console.error("[V2 Connect] billingPortal error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

  // ─── V2 Thin-Event Webhook ──────────────────────────────────────────────
  // This endpoint handles Stripe V2 thin events for connected account updates.
  // See stripe-connect-v2.ts for Stripe CLI and Dashboard setup instructions.
  app.post(
    "/api/webhooks/stripe-connect-v2",
    async (req: Request, res: Response) => {
      const sig = req.headers["stripe-signature"] as string;
      if (!sig) return res.status(400).send("Missing stripe-signature header");

      // Use rawBody if available (set by the body-parser middleware upstream),
      // otherwise fall back to a JSON stringified body.
      // Note: the rawBody property is attached by the express JSON middleware
      // configured in server/index.ts (same pattern used by the existing
      // /api/webhooks/stripe endpoint at line ~7457).
      const reqWithRaw = req as Request & { rawBody?: string };
      const rawBody = reqWithRaw.rawBody ?? JSON.stringify(req.body);

      let thinEvent: { id: string; type: string };
      try {
        thinEvent = parseV2ThinEvent(rawBody, sig);
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error("[V2 Webhook] Signature verification failed:", msg);
        return res.status(400).send(`Webhook Error: ${msg}`);
      }

      const [alreadyProcessedV2] = await db.select({ id: stripeEvents.id }).from(stripeEvents).where(eq(stripeEvents.id, thinEvent.id)).limit(1);
      if (alreadyProcessedV2) {
      ...
      try {
        ...
        if (thinEvent.type === "v2.core.account[requirements].updated") {
          // The connected account's requirements changed.  Log the summary and
          // take any necessary action (e.g., notify the user to complete onboarding).
          const accountId = fullEvent?.data?.id;
          const summary = fullEvent?.data?.requirements?.summary;
          console.log(`[V2 Webhook] Account requirements updated for ${accountId}: ${summary}`);

          if (accountId) {
            // Optionally sync the subscription status back from Stripe.
            const record = await storage.getConnectedAccountByStripeId(accountId);
            if (record) {
              // Touch the record so the updatedAt timestamp reflects the change.
              await storage.updateConnectedAccount(record.id, {});
            }
          }
        } else if (
          // V2 capability status events use bracket notation for the configuration
          // namespace. The merchant configuration emits:
          //   v2.core.account[configuration.merchant].capability_status_updated
          // Guard with startsWith so partial namespace matches are caught too.
          thinEvent.type === "v2.core.account[configuration.merchant].capability_status_updated" ||
          thinEvent.type.startsWith("v2.core.account[configuration.") &&
            thinEvent.type.endsWith("].capability_status_updated")
        ) {
          // A capability (e.g. card_payments) changed status on the merchant or
          // customer configuration.
          const accountId = fullEvent?.data?.account;
          const capability = fullEvent?.data?.capability;
          const status = fullEvent?.data?.status;
          console.log(`[V2 Webhook] Capability ${capability} → ${status} for account ${accountId}`);
        } else {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[V2 Webhook] Error processing event ${thinEvent.id}:`, msg);
      }
    }
  );

  // ─── V1 Subscription Webhook ────────────────────────────────────────────
  // Handles standard Stripe V1 lifecycle events for connected-account platform
  // subscriptions.  We use customer_account (not customer) to identify the
  // connected account because platform subscriptions are tied to accounts,
  // not customer objects.
  //
  // Stripe CLI (local testing only):
  //   stripe listen --forward-to localhost:5000/api/webhooks/stripe-v1-subscriptions \
  //     --events "customer.subscription.created,customer.subscription.updated,customer.subscription.deleted,invoice.payment_succeeded,invoice.payment_failed"
  app.post(
    "/api/webhooks/stripe-v1-subscriptions",
    async (req: Request, res: Response) => {
      const sig = req.headers["stripe-signature"] as string;
      if (!sig) return res.status(400).send("Missing stripe-signature header");

      const reqWithRawV1 = req as Request & { rawBody?: string };
      const rawBody = reqWithRawV1.rawBody ?? JSON.stringify(req.body);

      // Use a typed interface for V2-style subscription objects where
      // customer_account (not customer) identifies the connected account.
      interface V2SubscriptionObject {
        id: string;
        status: string;
        customer_account?: string;
      }
      interface V2InvoiceObject {
        id: string;
        customer_account?: string;
      }
      ...
      try {
        event = constructV1WebhookEvent(rawBody, sig);
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
      ...
      const [alreadyProcessedV1] = await db.select({ id: stripeEvents.id }).from(stripeEvents).where(eq(stripeEvents.id, event.id)).limit(1);
      if (alreadyProcessedV1) {
      ...
      try {
        // customer_account is the Stripe account ID of the connected account
        // that owns the subscription (as opposed to a regular customer ID).
        // We cast to our typed interface to access this V2 field safely.

        // Helper: look up the connected account record by the Stripe account ID
        // found in customer_account.
        const resolveRecord = async (accountId?: string) => {
          if (!accountId) return null;
          return storage.getConnectedAccountByStripeId(accountId);
        };

        switch (event.type) {
          case "customer.subscription.created": {
            // A new platform subscription was created for a connected account.
            const obj = event.data.object as unknown as V2SubscriptionObject;
            const accountId = obj.customer_account;
            const subscriptionId = obj.id;
            console.log(`[V1 Sub Webhook] Subscription created: ${subscriptionId} for account ${accountId}`);
            const record = await resolveRecord(accountId);
            if (record) {
              await storage.updateConnectedAccount(record.id, {
                subscriptionStatus: "active",
          ...
          case "customer.subscription.updated": {
            // Subscription status changed (e.g. trialing → active, active → past_due).
            const subObj = event.data.object as unknown as V2SubscriptionObject;
            const accountId = subObj.customer_account;
            const subscriptionId = subObj.id;
            const status = subObj.status; // Stripe subscription status string
            console.log(`[V1 Sub Webhook] Subscription updated: ${subscriptionId} → ${status} for account ${accountId}`);
            const record = await resolveRecord(accountId);
            if (record) {
              await storage.updateConnectedAccount(record.id, {
          ...
          case "customer.subscription.deleted": {
            ...
            const subObj = event.data.object as unknown as V2SubscriptionObject;
            const accountId = subObj.customer_account;
            console.log(`[V1 Sub Webhook] Subscription deleted for account ${accountId}`);
            const record = await resolveRecord(accountId);
            if (record) {
              await storage.updateConnectedAccount(record.id, {
          ...
          case "invoice.payment_succeeded": {
            ...
            const invObj = event.data.object as unknown as V2InvoiceObject;
            const accountId = invObj.customer_account;
            console.log(`[V1 Sub Webhook] Invoice paid for account ${accountId}`);
            const record = await resolveRecord(accountId);
            if (record && record.subscriptionStatus !== "active") {
              // Restore status if it was past_due.
              await storage.updateConnectedAccount(record.id, { subscriptionStatus: "active" });
            }
            break;
          }

          case "invoice.payment_failed": {
            // A subscription invoice payment failed — mark past_due.
            const invObj = event.data.object as unknown as V2InvoiceObject;
            const accountId = invObj.customer_account;
            console.log(`[V1 Sub Webhook] Invoice payment failed for account ${accountId}`);
            const record = await resolveRecord(accountId);
            if (record) {
              await storage.updateConnectedAccount(record.id, { subscriptionStatus: "past_due" });
            }
            break;
          }
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[V1 Sub Webhook] Error processing event ${event.id}:`, msg);
      }
    }
  );
