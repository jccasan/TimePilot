  ...
  app.post("/api/invoices/consolidate", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const allLineItems = submittedItems.map((mi: any) => {
        const qty = parseFloat(mi.quantity || "1");
      ...
      await auditLog(userId, companyId, "invoice.consolidated", { invoiceId: invoice.id, consolidatedFrom: voidedIds, lineItemCount: allLineItems.length });
      res.json(invoice);
    } catch (err) { handleError(res, err); }
  });
