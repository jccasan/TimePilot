...
export const priceRecommendationSourceEnum = pgEnum("price_recommendation_source", [
  "manual",
  "auto",
  "ai_optimizer",
]);
...
export const companies = pgTable("companies", {
  ...
  voiceAreaCodePreference: varchar("voice_area_code_preference", { length: 3 }),
  voiceNumberPortingStatus: varchar("voice_number_porting_status", { length: 20 }),
  ...
  maxRouteDurationHours: real("max_route_duration_hours").default(7.5),
  routePlanningMode: varchar("route_planning_mode", { length: 10 }).notNull().default("stops"),
  avgMinutesPerStop: integer("avg_minutes_per_stop").default(12),
  minRouteDurationHours: real("min_route_duration_hours").default(1),
...
export const contacts = pgTable(
  "contacts",
  {
    ...
    depositInvoiceId: varchar("deposit_invoice_id", { length: 255 }),
    billingOnboardingStage: varchar("billing_onboarding_stage", { length: 30 }).default("none"),
...
export const contactTags = pgTable(
  ...
  (table) => [unique().on(table.contactId, table.tagId)]
);
...
export const routes = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("idx_routes_company_date")
...
export const jobAddOns = pgTable(
  "job_add_ons",
  {
    ...
    servicePricingId: varchar("service_pricing_id")
      .notNull()
      .references(() => servicePricing.id, { onDelete: "cascade" }),
    name: varchar("name", { length: 255 }).notNull(),
    price: decimal("price", { precision: 10, scale: 2 }).notNull(),
    isActive: boolean("is_active").notNull().default(true),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [index("idx_jao_job").on(table.jobId)]
);

export const servicePlanAddOns = pgTable(
  ...
  (table) => [index("idx_spa_plan").on(table.servicePlanId)]
);
...
export const visits = pgTable(
  ...
  (table) => [
    ...
    unique("visits_service_plan_id_scheduled_date_unique").on(
      table.servicePlanId,
      table.scheduledDate
    ),
  ]
...
export const invoices = pgTable(
  "invoices",
  {
    ...
    stripeProcessingFeeCents: integer("stripe_processing_fee_cents"),
    ...
    isOnboardingInvoice: boolean("is_onboarding_invoice").notNull().default(false),
...
export const invoiceLineItems = pgTable("invoice_line_items", {
  ...
  servicePricingId: varchar("service_pricing_id").references(() => servicePricing.id, {
    onDelete: "set null",
  }),
...
export const automationRules = pgTable(
  ...
  (table) => [index("idx_ar_company").on(table.companyId)]
);
...
export const serviceBillingRules = pgTable(
...
export const servicePackages = pgTable(
  ...
  (table) => [index("idx_spkg_company").on(table.companyId)]
);
...
export const jobAddOnRelations = relations(jobAddOns, ({ one }) => ({
  ...
  servicePricing: one(servicePricing, {
    fields: [jobAddOns.servicePricingId],
...
export const servicePlanAddOnRelations = relations(servicePlanAddOns, ({ one }) => ({
  servicePlan: one(servicePlans, {
    fields: [servicePlanAddOns.servicePlanId],
    references: [servicePlans.id],
  }),
  servicePricing: one(servicePricing, {
    fields: [servicePlanAddOns.servicePricingId],
    references: [servicePricing.id],
  }),
}));
...
export const vacationHoldRelations = relations(vacationHolds, ({ one }) => ({
  servicePlan: one(servicePlans, {
    fields: [vacationHolds.servicePlanId],
    references: [servicePlans.id],
  }),
}));
...
export const insertServiceBillingRuleSchema = createInsertSchema(serviceBillingRules).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertJobAddOnSchema = createInsertSchema(jobAddOns).omit({
  id: true,
  createdAt: true,
});
...
export const insertVacationHoldSchema = createInsertSchema(vacationHolds).omit({
  id: true,
  createdAt: true,
});
...
export const saasCostsMonthly = pgTable(
  "saas_costs_monthly",
  {
    id: varchar("id")
      .primaryKey()
...
export const insertAccountDailyMetricsSchema = createInsertSchema(accountDailyMetrics).omit({
  id: true,
  createdAt: true,
});
export const insertSaasCostsMonthlySchema = createInsertSchema(saasCostsMonthly).omit({
  id: true,
  createdAt: true,
});
export const insertCostConfigSchema = createInsertSchema(costConfig).omit({
  id: true,
  updatedAt: true,
});
...
export const adminUsers = pgTable("admin_users", {
  ...
  previousPasswordHashes: text("previous_password_hashes")
    .array()
    .notNull()
    .default(sql`'{}'::text[]`),
...
export const roverTicketTypeEnum = pgEnum("rover_ticket_type", [
  "bug",
  "feature_request",
  "question",
]);
export const roverTicketStatusEnum = pgEnum("rover_ticket_status", [
  "open",
  "in_progress",
  "resolved",
  "closed",
...
export const roverTickets = pgTable(
  "rover_tickets",
  {
    id: varchar("id")
      .primaryKey()
...
export const insertRoverTicketSchema = createInsertSchema(roverTickets).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const invoicePaymentRelations = relations(invoicePayments, ({ one }) => ({
  ...
  importRun: one(importRuns, {
    fields: [invoicePayments.importRunId],
...
export const priceRecommendationRelations = relations(priceRecommendations, ({ one }) => ({
  ...
  property: one(properties, {
    fields: [priceRecommendations.propertyId],
    references: [properties.id],
  }),
  route: one(routes, { fields: [priceRecommendations.routeId], references: [routes.id] }),
}));

export const profitabilityStatusEnum = pgEnum("profitability_status", [
  "profitable",
  "marginal",
  "unprofitable",
]);
...
export const profitabilitySnapshots = pgTable(
  "profitability_snapshots",
  {
    ...
    profitMarginPct: decimal("profit_margin_pct", { precision: 8, scale: 2 })
      .notNull()
      .default("0"),
...
export const insertProfitabilitySnapshotSchema = createInsertSchema(profitabilitySnapshots).omit({
  id: true,
  createdAt: true,
});
...
export const competitorPricing = pgTable(
  "competitor_pricing",
  {
    id: varchar("id")
      .primaryKey()
...
export const estimates = pgTable(
  "estimates",
  {
    ...
    description: text("description").notNull(),
    items: jsonb("items")
      .$type<Array<{ description: string; quantity: number; unitPrice: string; total: string }>>()
      .notNull(),
...
export const serviceChangeRequests = pgTable(
...
export const insertServiceChangeRequestSchema = createInsertSchema(serviceChangeRequests).omit({
  id: true,
  createdAt: true,
});
...
export const qboSyncLogs = pgTable(
  "qbo_sync_logs",
  {
    id: varchar("id")
      .primaryKey()
...
export const insertQboSyncLogSchema = createInsertSchema(qboSyncLogs).omit({
  id: true,
  createdAt: true,
});
...
export const DEFAULT_QUOTE_DEFAULTS: QuoteDefaults = {
  ...
  commercialDensityDiscount2x: 0.1,
...
export const messageRouting = pgTable(
  ...
  (table) => [
    ...
    unique("uq_mr_shared_customer_company").on(
      table.sharedNumber,
      table.customerPhone,
      table.companyId
    ),
...
export const insertMessageRoutingSchema = createInsertSchema(messageRouting).omit({
  id: true,
  createdAt: true,
});
...
export const messageExceptions = pgTable(
  "message_exceptions",
  {
    id: varchar("id")
      .primaryKey()
    ...
    candidateCompanyIds: text("candidate_company_ids")
      .array()
      .default(sql`'{}'`),
    resolvedAt: timestamp("resolved_at"),
    resolvedBy: varchar("resolved_by").references(() => users.id),
    resolvedCompanyId: varchar("resolved_company_id").references(() => companies.id, {
      onDelete: "set null",
    }),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
...
export const insertMessageExceptionSchema = createInsertSchema(messageExceptions).omit({
  id: true,
  createdAt: true,
});
...
export const systemMessageSeverityEnum = pgEnum("system_message_severity", [
  "info",
  "warning",
  "error",
]);
...
export const stripeEvents = pgTable(
  ...
  (table) => [index("idx_stripe_events_processed").on(table.processedAt)]
);
...
export const insertBusinessAssessmentSchema = createInsertSchema(businessAssessments).omit({
  id: true,
  createdAt: true,
});
...
export const quoteFormEvents = pgTable(
  "quote_form_events",
  {
    id: varchar("id")
      .primaryKey()
...
export const insertQuoteFormEventSchema = createInsertSchema(quoteFormEvents).omit({
  id: true,
  createdAt: true,
});
...
export const errorReportSeverityEnum = pgEnum("error_report_severity", [
  "low",
  "medium",
  "high",
  "critical",
...
export const errorFixTasks = pgTable(
  "error_fix_tasks",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    errorReportId: varchar("error_report_id")
      .notNull()
      .references(() => errorReports.id, { onDelete: "cascade" }),
    title: text("title").notNull(),
    status: varchar("status", { enum: ["open", "done"] })
      .notNull()
      .default("open"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [index("idx_eft_report").on(table.errorReportId)]
);

export const insertErrorFixTaskSchema = createInsertSchema(errorFixTasks).omit({
  id: true,
  createdAt: true,
});
...
export const reviewResponses = pgTable(
  ...
  (table) => [index("idx_rr_token").on(table.tokenId)]
);
...
export const geocodeCacheTable = pgTable(
  ...
  (table) => [index("idx_geocode_cache_cached_at").on(table.cachedAt)]
);

export type GeocodeCache = typeof geocodeCacheTable.$inferSelect;

export const autocompleteCacheTable = pgTable(
  "autocomplete_cache",
  {
  ...
  (table) => [index("idx_autocomplete_cache_cached_at").on(table.cachedAt)]
);

export type AutocompleteCache = typeof autocompleteCacheTable.$inferSelect;

export const retellWebhookRepairs = pgTable(
  "retell_webhook_repairs",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
